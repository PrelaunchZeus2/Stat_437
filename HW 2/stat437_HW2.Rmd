...
summary_stats = summary_stats %>%
  mutate(
    carrier_new = case_when(
      carrier == "AA" ~ expression(alpha[1]),
      carrier == "DL" ~ expression(beta[1*","*2]),
      carrier == "UA" ~ expression(gamma^"[0]")
    )
  )
p1 = p + geom_line() 
p1
...
plotly3 = plot_ly(mpg, x = ~displ, y = ~hwy, color = ~cyl, type = "scatter") %>% layout(xaxis=list(title="engine displacement in litres"), yaxis=list(title="highway miles per gallon"))
plotly3
```
