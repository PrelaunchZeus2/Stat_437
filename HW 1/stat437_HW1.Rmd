...
flights1 = flights %>% filter(month %in% c(12, 1, 2, 6, 7, 8), carrier %in% c("UA", "AA", "DL"), distance > 700)
...
arr_delay.summary_stats_plot = ggplot(data = summary_stats, aes(x = month, y = mean_arr_delay, shape = carrier, color = mean_distance)) + geom_point(size = 3) + geom_text(aes(label = carrier), hjust = 0.5, vjust = -0.5, size = 3) + scale_color_gradient(low = "green", high = "red")
...
```{r, 2}
mpg.fourfactorplot = ggplot(mpg, aes(x = displ, y = hwy)) + theme_bw() + geom_point(aes(colour=class,shape=trans)) + scale_shape_manual(values = 1:length(unique(mpg$trans))) + facet_grid(drv~cyl)
mpg.fourfactorplot
```
