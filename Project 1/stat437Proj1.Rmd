...
gap = clusGap(fiftygeneset, FUN = kmeans, K.max = 10, B = 200, iter.max = 100)
...
vis$Type = labels1 %>% filter(rownames(labels1) %in% rownames(labels1)) %>% pull(Class)
...
optimal_k_250 = maxSE(gap$Tab[, "gap"], gap$Tab[, "SE.sim"], method="Tibs2001SEmax")
print(optimal_k_250)

k_means = kmeans(twofiftygeneset, optimal_k_250, iter.max = 100, nstart = 25, algorithm = c("Hartigan-Wong"))
