...
gexp2 = gene_data %>% select_if(~ sum(. == 0) <= 300)
...
gap = clusGap(fiftygeneset, FUNcluster = kmeans, K.max = 10, B = 200, iter.max = 100, nstart = 20)
optimal_k = maxSE(f = gap$Tab[, "gap"], SE.f = gap$Tab[, "SE.sim"], method = "Tibs2001SEmax", SE.factor = 1)
...
plot = ggplot(vis, aes(x = gene_19103, y = gene_6277)) + xlab("Gene 19103 Expression") + ylab("Gene 6277 Expression") + theme_bw() + geom_point(aes(shape = Type, color = cluster)) + ggtitle("Clustering results using clusGap to estimate cluster number")
plot
```
...
plot3 = ggplot(vis2, aes(x = gene_19103, y = gene_6277)) + xlab("Gene 19103 Expression") + ylab("Gene 6277 Expression") + theme_bw() + geom_point(aes(shape = Type, color = cluster)) + ggtitle("Clustering results using clusGap to estimate cluster number")
plot3
