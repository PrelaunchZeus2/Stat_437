...
khat = which(testErrors[1,] == min(testErrors[1,]))
cl = y_train[,1]
best_knn = knn(x_train, x_test, cl, khat)
n_errors = sum(1 - as.numeric(as.data.frame(best_knn)==y_test))
best_test_error = n_errors/length(y_test)
print(best_test_error)
```
```{r, knn plotting}
x_test$carrier = factor(y_test[,1])
x_test$estimated_carrier = best_knn

p = ggplot(x_test, aes(arr_delay, dep_delay)) + geom_point(aes(shape = estimated_carrier, color = carrier)) + theme_bw()
p
```
