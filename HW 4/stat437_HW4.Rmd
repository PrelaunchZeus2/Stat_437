...
flights = flights %>% filter(carrier %in% c('UA', 'AA', 'DL')) #filter to include only the 3 carriers
flights = flights %>% select(dep_delay, arr_delay, distance, carrier) #get the three features
FiveHundredIndices = sample(1:dim(flights)[1], size = 500, replace = FALSE)
flights = flights[FiveHundredIndices,] #get 500 randomly (with seed 123) sampled flight observations
...
flights = scale(flights) #scale observations

train_size = floor(0.7 * nrow(data))
train_indices = sample(seq_len(nrow(data)), size = train_size)

#make train test sets with indices
train = flights[train_indices,]
test = flights[-train_indices,]
```
